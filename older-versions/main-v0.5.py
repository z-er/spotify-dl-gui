# main.py — spotify-dl GUI v0.5
import sys, shlex, re, os, subprocess, platform, ctypes, time, shutil, json
from pathlib import Path
from shutil import which as _which
from datetime import datetime

from PySide6.QtCore import Qt, QProcess, QSettings, QTimer, QIODevice
from PySide6.QtGui import QPalette, QColor, QIcon, QAction, QCloseEvent
from PySide6.QtWidgets import (
    QApplication, QWidget, QVBoxLayout, QHBoxLayout, QLabel, QLineEdit,
    QTextEdit, QPushButton, QFileDialog, QSpinBox, QDoubleSpinBox, QComboBox, QMessageBox,
...
class SettingsDialog(QDialog):
    def __init__(self, parent, settings: QSettings):
        ...
        self.minimize_tray_chk = QCheckBox("Minimize to tray on close (keep running)")
        ...
        # Integrity checks
        self.integrity_chk = QCheckBox("Flag incomplete downloads")
        self.integrity_min_mb = QDoubleSpinBox()
        self.integrity_min_mb.setRange(0.1, 50.0)
        self.integrity_min_mb.setSingleStep(0.1)
        self.integrity_min_mb.setSuffix(" MB")
        self.integrity_min_mb.setValue(1.0)
        self.integrity_min_sec_chk = QCheckBox("Also flag when duration is under")
        self.integrity_min_sec = QSpinBox()
        self.integrity_min_sec.setRange(1, 120)
        self.integrity_min_sec.setSuffix(" sec")
        self.integrity_min_sec.setValue(10)

        # M3U8 export
        self.m3u_export_chk = QCheckBox("Export M3U8 playlist per run")
        self.m3u_one_folder_chk = QCheckBox("If single album/dir, place M3U8 in that folder (otherwise use _playlists/)")
        ...
        layout.addWidget(self.minimize_tray_chk)
        layout.addSpacing(6)
        ...
        layout.addWidget(QLabel("Integrity"))
        row_integrity1 = QHBoxLayout()
        row_integrity1.addWidget(self.integrity_chk); row_integrity1.addStretch()
        row_integrity2 = QHBoxLayout()
        row_integrity2.addWidget(QLabel("Size threshold:")); row_integrity2.addWidget(self.integrity_min_mb); row_integrity2.addStretch()
        row_integrity3 = QHBoxLayout()
        row_integrity3.addWidget(self.integrity_min_sec_chk); row_integrity3.addWidget(self.integrity_min_sec); row_integrity3.addStretch()
        layout.addLayout(row_integrity1); layout.addLayout(row_integrity2); layout.addLayout(row_integrity3)
        layout.addSpacing(6)

        layout.addWidget(QLabel("M3U8 Export"))
        layout.addWidget(self.m3u_export_chk)
        layout.addWidget(self.m3u_one_folder_chk)
        layout.addSpacing(6)
        ...
        self.minimize_tray_chk.setChecked(settings.value("minimize_to_tray", "true") == "true")
        ...
        self.integrity_chk.setChecked(settings.value("integrity_flag", "true") == "true")
        try: self.integrity_min_mb.setValue(float(settings.value("integrity_min_mb", 1.0)))
        except Exception: self.integrity_min_mb.setValue(1.0)
        self.integrity_min_sec_chk.setChecked(settings.value("integrity_duration_flag", "false") == "true")
        try: self.integrity_min_sec.setValue(int(settings.value("integrity_min_seconds", 10)))
        except Exception: self.integrity_min_sec.setValue(10)

        self.m3u_export_chk.setChecked(settings.value("m3u_export", "true") == "true")
        self.m3u_one_folder_chk.setChecked(settings.value("m3u_in_folder_when_single", "true") == "true")

        self.bin_edit.setText(settings.value("bin", ""))
        self._update_preview()

    def _update_preview(self):
        demo = {"artist":"David Bowie","album":"Heroes","title":"Heroes","track":1,"disc":1,"year":1977,"ext":".flac","filename":"David Bowie - Heroes.flac"}
        class FmtDict(dict):
            def __missing__(self, k): return "{"+k+"}"
        try: preview = self.template_edit.text().format_map(FmtDict(demo)).replace("//","/").strip("/\\")
        except Exception: preview = "(invalid template)"
    ...
    def pick_bin(self):
        ...
        if path: self.bin_edit.setText(path)

    def apply(self):
        self.settings.setValue("open_when_done", "true" if self.open_when_done_chk.isChecked() else "false")
        self.settings.setValue("minimize_to_tray", "true" if self.minimize_tray_chk.isChecked() else "false")
        ...
        self.settings.setValue("integrity_flag", "true" if self.integrity_chk.isChecked() else "false")
        self.settings.setValue("integrity_min_mb", str(self.integrity_min_mb.value()))
        self.settings.setValue("integrity_duration_flag", "true" if self.integrity_min_sec_chk.isChecked() else "false")
        self.settings.setValue("integrity_min_seconds", str(self.integrity_min_sec.value()))
        self.settings.setValue("m3u_export", "true" if self.m3u_export_chk.isChecked() else "false")
        self.settings.setValue("m3u_in_folder_when_single", "true" if self.m3u_one_folder_chk.isChecked() else "false")
...
class HistoryDialog(QDialog):
    ...
    def open_dest(self):
        ...
        if d and Path(d).exists():
            try:
                if sys.platform.startswith("win"): os.startfile(d)  # type: ignore
...
class QueueRow(QWidget):
    def __init__(self, url: str, status: QStatus = QStatus.PENDING):
        ...
        self.lbl_badge = QLabel(QUEUE_BADGE[status]); self.lbl_badge.setFixedWidth(24)
        self.lbl_url = QLabel(url)
        self.lbl_url.setTextInteractionFlags(Qt.TextSelectableByMouse)
        self.lbl_url.setWordWrap(False); self.lbl_url.setMinimumWidth(240); self.lbl_url.setToolTip(url)
        self.lbl_status = QLabel(status.value); self.lbl_status.setProperty("class", "muted")
        ...
        row.setContentsMargins(8, 6, 8, 6); row.setSpacing(10)
    ...
    def set_status(self, status: QStatus):
        ...
        if status in (QStatus.OK, QStatus.FAIL, QStatus.SKIPPED):
            self.progress.setValue(0 if status != QStatus.OK else 100)
...
class SpotifyDLGui(QWidget):
    def __init__(self):
        ...
        # Header
        header = QLabel(f"spotify-dl — simple GUI  •  {APP_VER}")
        header.setStyleSheet("font-size: 18px; font-weight: 600;")

        self.settings_btn = QPushButton("Settings"); self.settings_btn.clicked.connect(self.open_settings)
        self.history_btn = QPushButton("History"); self.history_btn.clicked.connect(self.open_history)

        header_row = QHBoxLayout()
        header_row.addWidget(header); header_row.addStretch()
        header_row.addWidget(self.history_btn); header_row.addWidget(self.settings_btn)

        # LEFT: Queue panel
        self.queue_list = QListWidget()
        self.queue_list.setSelectionMode(QListWidget.ExtendedSelection)
        self.queue_list.setDragEnabled(True); self.queue_list.setAcceptDrops(True)
        self.queue_list.setDragDropMode(QListWidget.InternalMove)
        self.queue_list.setDefaultDropAction(Qt.MoveAction)
        self.queue_list.setSpacing(6); self.queue_list.setMinimumWidth(400)

        self.btn_q_add = QPushButton("Add URLs ➜ Queue"); self.btn_q_add.clicked.connect(self._queue_add_from_textbox)
        self.btn_q_remove = QPushButton("Remove"); self.btn_q_remove.clicked.connect(self._queue_remove_selected)
        self.btn_q_clear = QPushButton("Clear"); self.btn_q_clear.clicked.connect(self._queue_clear)
        self.btn_q_up = QPushButton("↑"); self.btn_q_up.clicked.connect(lambda: self._queue_nudge(-1))
        self.btn_q_down = QPushButton("↓"); self.btn_q_down.clicked.connect(lambda: self._queue_nudge(+1))

        # Run/Pause/Stop (LEFT)
        self.run_btn = QPushButton("Run")
        self.run_btn.clicked.connect(self.start_queue); self.run_btn.setDefault(True)
        self.pause_btn = QPushButton("Pause after current"); self.pause_btn.clicked.connect(self._toggle_pause)
        self.pause_btn.setEnabled(False)
        self.stop_btn = QPushButton("Stop"); self.stop_btn.clicked.connect(self.stop_queue); self.stop_btn.setEnabled(False)

        # Re-run helpers
        self.rerun_failed_btn = QPushButton("Re-run Failed")
        self.rerun_failed_btn.clicked.connect(lambda: self._rerun_from_history("fail"))
        self.rerun_sus_btn = QPushButton("Re-run Suspects")
        self.rerun_sus_btn.clicked.connect(lambda: self._rerun_from_history("suspect"))

        # Clipboard direct-add toggle (auto-queue)
        self.auto_clip_chk = QCheckBox("Auto-add clipboard links")
        self.auto_clip_chk.setChecked(True)

        qctrl_top = QHBoxLayout()
        qctrl_top.addWidget(self.btn_q_add)
        qctrl_top.addStretch()
        qctrl_top.addWidget(self.btn_q_up)
        qctrl_top.addWidget(self.btn_q_down)
        qctrl_top.addSpacing(8)
        qctrl_top.addWidget(self.btn_q_remove)
        qctrl_top.addWidget(self.btn_q_clear)

        qctrl_run = QHBoxLayout()
        qctrl_run.addWidget(self.run_btn)
        qctrl_run.addWidget(self.pause_btn)
        qctrl_run.addWidget(self.stop_btn)
        qctrl_run.addStretch()
        qctrl_run.addWidget(self.rerun_failed_btn)
        qctrl_run.addWidget(self.rerun_sus_btn)

        left = QVBoxLayout()
        left.addLayout(qctrl_top)
        left.addWidget(self.queue_list, 1)
        left.addSpacing(6)
        left.addWidget(self.auto_clip_chk)
        left.addLayout(qctrl_run)

        # RIGHT: Form + live tail
        self.urls_edit = QTextEdit(); self.urls_edit.setPlaceholderText("Optional staging: paste URLs here (one per line) and click “Add URLs ➜ Queue”")
        self.urls_edit.setAcceptRichText(False)

        self.dest_edit = QLineEdit(); self.dest_edit.setPlaceholderText("Choose a destination folder")
        btn_dest = QPushButton("Browse"); btn_dest.clicked.connect(self.pick_dest)

        dest_row = QHBoxLayout()
        dest_row.addWidget(QLabel("Destination")); dest_row.addSpacing(8)
        dest_row.addWidget(self.dest_edit, 1); dest_row.addWidget(btn_dest)

        self.format_combo = QComboBox(); self.format_combo.addItems(["flac", "mp3", "m4a", "opus"])
        self.parallel_spin = QSpinBox(); self.parallel_spin.setRange(1, 32); self.parallel_spin.setValue(5)
        self.force_chk = QCheckBox("Force re-download")

        opts_row = QHBoxLayout()
        opts_row.addWidget(QLabel("Format")); opts_row.addWidget(self.format_combo)
        opts_row.addSpacing(16); opts_row.addWidget(QLabel("Parallel")); opts_row.addWidget(self.parallel_spin)
        opts_row.addSpacing(16); opts_row.addWidget(self.force_chk)
        opts_row.addStretch()

        self.extra_args = QLineEdit(); self.extra_args.setPlaceholderText("Optional extra flags (advanced) e.g. --something")

        # Live tail viewer + backoff banner
        self.tail_view = QTextEdit()
        ...
        # Persistent terminal button (right column utility)
        ...
        util_row.addWidget(self.toggle_terminal_btn)
        util_row.addStretch()
        ...
        footer = QHBoxLayout(); footer.addStretch(); footer.addWidget(self.bin_pill)

        right = QVBoxLayout()
        right.addLayout(header_row); right.addSpacing(4); right.addWidget(Line()); right.addSpacing(4)
        right.addWidget(QLabel("Tracks / Playlists / Albums"))
        right.addWidget(self.urls_edit)
        right.addLayout(dest_row); right.addSpacing(4); right.addLayout(opts_row)
        right.addSpacing(6); right.addWidget(QLabel("Extra flags")); right.addWidget(self.extra_args)
        right.addSpacing(8); right.addWidget(Line())
        right.addLayout(util_row)
        right.addSpacing(6)
        right.addWidget(self.tail_view, 1)
        right.addSpacing(6)
        ...
        layout = QVBoxLayout(self)
        split = QHBoxLayout()
        split.addLayout(left, 0); split.addSpacing(10); split.addLayout(right, 1)
        layout.addLayout(split, 1)

        # System tray (notifications + icon + double-click restore)
        self.tray = None
        if QSystemTrayIcon.isSystemTrayAvailable():
            ...
            tray_menu = QMenu()
            act_show = QAction("Show"); act_show.triggered.connect(self.show_normal_from_tray)
            act_exit = QAction("Exit"); act_exit.triggered.connect(QApplication.instance().quit)
            tray_menu.addAction(act_show); tray_menu.addSeparator(); tray_menu.addAction(act_exit)
        ...
        self._queue_agg = {"moved":0, "replaced":0, "deleted":0, "skipped":0, "ok":0, "fail":0, "suspect":0}
        ...
        self._log_buffer: list[str] = []          # filled from temp file at end
        self._job_temp_log: str | None = None     # raw stdout+stderr per job
        self._job_outputs: list[dict] = []
        self._job_suspects: list[dict] = []
        self._job_start_iso: str = ""
        self._current_url: str = ""
        self._job_stats = {"moved":0, "replaced":0, "deleted":0, "skipped":0}

        # Live tail timer
        self._tail_timer = QTimer(self); self._tail_timer.setInterval(500)
        self._tail_timer.timeout.connect(self._tail_tick)
        self._tail_pos = 0

        # Clipboard watcher (auto-add)
    ...
    def update_bin_pill(self):
        ...
        if ok:
            ...
            tooltip = ("Place 'spotify-dl(.exe)' next to the app, or set a custom path in Settings, "
                       "or add to PATH.")
        self.bin_pill.setText(text); self.bin_pill.setToolTip(tooltip)
        if ok:   bg = "#1b2a22"; border = "#2a2f39"; textc = "#e6eaf2"
        else:    bg = "#2a1d1d"; border = "#f7768e"; textc = "#fbcaca"
    ...
    def _read_tags(self, path: Path) -> dict:
        tags = {"artist": "", "album": "", "title": path.stem, "track": 0, "disc": 1, "year": 0, "ext": path.suffix, "filename": path.name}
        ...
        try:
            if ext == ".mp3":
                ...
                tags["album"] = alb or ""; tags["artist"] = art or ""; tags["title"] = ttl or tags["title"]
    ...
    def _maybe_extract_cover(self, audio_path: Path, album_dir: Path):
        ...
        try:
            ...
            if ext == ".mp3":
                id3 = ID3(str(audio_path)); apics = id3.getall("APIC")
                if apics:
                    data = apics[0].data
                    is_png = apics[0].mime == "image/png" or (data[:8] == b"\x89PNG\r\n\x1a\n")
            elif ext in {".m4a", ".mp4"}:
                mp = MP4(str(audio_path)); covr = mp.tags.get("covr")
                if covr:
                    pic = covr[0]
            ...
            with open(album_dir / fname, "wb") as f: f.write(data)
        except Exception:
            pass
    ...
    def _maybe_flag_suspect(self, final_path: Path, tags: dict):
        if self.settings.value("integrity_flag", "true") != "true":
            return
        try:
        ...
        reasons = []
        try:
            min_mb = float(self.settings.value("integrity_min_mb", 1.0))
        except Exception:
            min_mb = 1.0
        if size_mb >= 0 and size_mb < min_mb:
            reasons.append(f"size {size_mb:.2f} MB < {min_mb:.2f} MB")

        dur = None
        if self.settings.value("integrity_duration_flag", "false") == "true":
            try: min_sec = int(self.settings.value("integrity_min_seconds", 10))
            except Exception: min_sec = 10
            dur = self._audio_duration_seconds(final_path)
            if dur is not None and dur < float(min_sec):
                reasons.append(f"duration {dur:.1f}s < {min_sec}s")
        ...
        if reasons:
            ...
            self._job_suspects.append(entry)
            self._queue_agg["suspect"] += 1
    ...
    def _organizer_move(self, file_path: Path, dest_root: Path) -> None:
        ...
        if target.exists():
            if dup_resolve:
                try:
                    src_size = file_path.stat().st_size; dst_size = target.stat().st_size
                except Exception:
                    src_size = -1; dst_size = -1
                if src_size > dst_size:
                    temp_in_album = album_dir / (file_path.name + ".tmp.incoming")
                    try:
                        try: shutil.move(str(file_path), str(temp_in_album))
                        except Exception:
                            shutil.copy2(str(file_path), str(temp_in_album)); file_path.unlink(missing_ok=True)
                        self._replace_file(temp_in_album, target)
                        tags = self._read_tags(target)
                        self._record_output(tags, target, stats_inc_key="replaced")
                        self._maybe_extract_cover(target, album_dir)
                        self._maybe_flag_suspect(target, tags)
                    finally:
                    ...
                    if dup_delete_smaller:
                        ...
                        self._job_stats["deleted"] += 1; self._queue_agg["deleted"] += 1
                    else:
                        self._job_stats["skipped"] += 1; self._queue_agg["skipped"] += 1
                return
            else:
                stem, ext = file_path.stem, file_path.suffix; n = 1
                while True:
        ...
        try:
            ...
            try:
                shutil.copy2(str(file_path), str(target)); file_path.unlink(missing_ok=True)
            except Exception:
                pass
        else:
            tags = self._read_tags(target)
            self._record_output(tags, target, stats_inc_key="moved")
            self._maybe_extract_cover(target, album_dir)
            self._maybe_flag_suspect(target, tags)
    ...
    # ---------- M3U8 export ----------
    def _maybe_write_m3u8(self, dest_dir: str):
        if self.settings.value("m3u_export", "true") != "true":
            return
        if not self._job_outputs:
            return
        # Determine placement: if all files share same parent, place M3U8 there (when setting allows). Else in dest/_playlists.
        parent_dirs = {str(Path(o["dest"]).parent) for o in self._job_outputs if o.get("dest")}
        target_dir = None
        if len(parent_dirs) == 1 and self.settings.value("m3u_in_folder_when_single", "true") == "true":
        ...
        try:
            with open(m3u_path, "w", encoding="utf-8", errors="ignore") as f:
                ...
                for o in self._job_outputs:
                    p = o.get("dest","")
                    if not p: continue
                    # EXTINF (optional): duration not known here cheaply; write artist - title
    ...
    def _write_log_file(self, dest_dir: str, url_index: int) -> str | None:
        try:
            ...
            sus_by_path = { s["dest"]: s for s in (self._job_suspects or []) }
            ...
            for o in self._job_outputs:
                ...
                suspect = sus_by_path.get(dest)
                suffix = f"  [SUSPECT: {suspect['reason']}]" if suspect else ""
                header_lines.append(f"  - {artist} – {title} [{album}]{suffix}")
                header_lines.append(f"    → {dest}  ({size_mb})")

            if self._job_suspects:
                header_lines.append("")
                header_lines.append(f"Suspect files ({len(self._job_suspects)}):")
                for s in self._job_suspects:
    ...
    def _add_from_textbox_and_clear(self):
    ...
    def _queue_add_urls(self, urls: list[str]):
        existing = {self._queue_item_row(i).url for i in range(self.queue_list.count())}
        added = 0
        for url in urls:
            ...
            added += 1
        if added:
            self._notify("Queued links", f"Added {added} link(s) to the queue.", 3000)
    ...
    def _queue_add_from_textbox(self):
        self._add_from_textbox_and_clear()
    ...
    def set_running(self, running: bool):
        for w in [self.urls_edit, self.dest_edit, self.format_combo, self.parallel_spin,
                  self.force_chk, self.extra_args, self.settings_btn, self.history_btn,
                  self.toggle_terminal_btn, self.btn_q_add, self.btn_q_remove, self.btn_q_clear, self.btn_q_up, self.btn_q_down]:
    ...
    def start_queue(self):
        ...
        # Add any staged URLs and clear staging
        if self.urls_edit.toPlainText().strip():
            self._add_from_textbox_and_clear()
        ...
        try:
            ...
            test = Path(dest) / ".write_test.tmp"
            test.write_text("ok", encoding="utf-8")
            test.unlink(missing_ok=True)
        except Exception:
            QMessageBox.critical(self, "Destination not writable", f"Cannot write to: {dest}")
            return
        ...
        self._persist_session()  # crash-safe resume
    ...
    def _build_base_args(self):
        ...
        # args += ["--verbose"]  # uncomment if your spotify-dl fork supports it
        return args

    def _schedule_next_job(self, delay_seconds: int = 0):
        self._pending_delay_ms = max(0, int(delay_seconds * 1000))
        if delay_seconds > 0:
            self._backoff_until = time.time() + delay_seconds
            self._update_backoff_countdown()
        else:
            self.backoff_label.clear()
        if self._paused: return
        self._maybe_start_next_after_delay()

    def _update_backoff_countdown(self):
        remain = int(getattr(self, "_backoff_until", 0) - time.time())
        if remain > 0:
            self.backoff_label.setText(f"Cooling down {remain}s to avoid rate limits…")
            QTimer.singleShot(1000, self._update_backoff_countdown)
        else:
            self.backoff_label.clear()
    ...
    def _maybe_start_next_after_delay(self):
        ...
        if self._pending_delay_ms > 0:
            delay = self._pending_delay_ms; self._pending_delay_ms = 0
    ...
    def _start_next_job(self):
        ...
        self._job_outputs = []; self._job_suspects = []
        self._job_stats = {"moved":0, "replaced":0, "deleted":0, "skipped":0}
        self._job_start_iso = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        self._log_buffer = []; self._run_started_at = time.time()
        self._last_job_output_count = 0; self._job_temp_log = None

        dest = self.dest_edit.text().strip()
        self._pre_run_files = self._list_audio_files(Path(dest)) if dest else set()

        try: exe = self.resolve_binary()
        except Exception as e:
            QMessageBox.critical(self, "Cannot start", str(e))
            self.stop_queue(); return
        ...
        if 0 <= self._queue_index < self.queue_list.count():
            ...
            roww.set_status(QStatus.RUNNING); roww.set_progress(0)
            self.queue_list.setCurrentRow(self._queue_index)

        # Prepare per-job temp raw-output file
        logs_dir = Path(dest) / "_logs" if dest else Path.cwd() / "_logs"
        try: logs_dir.mkdir(parents=True, exist_ok=True)
        except Exception: pass
        tmp_name = f"raw_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{self._queue_index+1:02d}.tmp"
        self._job_temp_log = str(logs_dir / tmp_name)

        # Start QProcess with merged output redirected to file
        self.proc_qt = QProcess(self)
        self.proc_qt.setProcessChannelMode(QProcess.MergedChannels)
        self.proc_qt.setStandardOutputFile(self._job_temp_log, QIODevice.Append)
        self.proc_qt.setProgram(exe); self.proc_qt.setArguments(args)
        ...
        if platform.system() == "Windows" and self.settings.value("persistent_terminal", "false") == "true":
            cmdline = " ".join([exe] + [shlex.quote(a) for a in args]); self.send_to_persistent(cmdline)

        # Persist session after starting job
        self._persist_session()

    def _tail_tick(self):
        if not self._job_temp_log: return
        try:
            ...
            if data:
                ...
                # feed UI
                self.tail_view.moveCursor(self.tail_view.textCursor().End)
                self.tail_view.insertPlainText(txt.replace("\r", "\n"))
                # parse % for progress
                for m in PERCENT_RE.finditer(txt):
                    pct = int(m.group(1))
                    if 0 <= self._queue_index < self.queue_list.count():
                        self._queue_item_row(self._queue_index).set_progress(pct)
        except Exception:
            pass
    ...
    def _notify(self, title: str, body: str, msec: int = 5000):
        if self.tray:
            self.tray.showMessage(title, body, QSystemTrayIcon.Information, msec)

    def _tray_activated(self, reason):
        if reason == QSystemTrayIcon.Trigger:  # single click / double click depending on platform
    ...
    def _on_job_finished(self, code, status):
        ...
        # Stop live tail and hide
        self._tail_timer.stop()
        self.tail_view.setVisible(False)

        # Pull raw output from temp file (if any)
        if self._job_temp_log and Path(self._job_temp_log).exists():
            try:
                ...
                try: raw_text = Path(self._job_temp_log).read_text(errors="ignore")
                except Exception: raw_text = ""
            raw_text = raw_text.replace("\r", "\n")
            self._log_buffer = [raw_text]
            try: Path(self._job_temp_log).unlink(missing_ok=True)
            except Exception: pass
        ...
        if int(code) == 0:
            try:
                ...
                # Write M3U8 if requested
                self._maybe_write_m3u8(dest)
                self._queue_agg["ok"] += 1
                if BACKOFF_RESET_ON_SUCCESS:
                    self._consecutive_failures = 0; self._backoff_index = 0
            except Exception:
                pass
        else:
        ...
        # Windows tray notification per URL
        ok = (int(code) == 0)
        sus_n = len(self._job_suspects)
        title = "Download complete" if ok else "Download failed"
        body = f"URL {self._queue_index+1}/{len(self._queue_urls)} — {'OK' if ok else 'Failed'}; files: {self._last_job_output_count}"
        if sus_n: body += f"; suspects: {sus_n}"
        if log_path: body += f"\nLog: {Path(log_path).name}"
        self._notify(title, body, 6000)
        ...
        # cleanup and advance
        self._pre_run_files = None; self._run_started_at = None; self._log_buffer = []
        self._queue_index += 1
    ...
    def stop_queue(self):
        if self.proc_qt and self.proc_qt.state() != QProcess.NotRunning:
            ...
            if not self.proc_qt.waitForFinished(2000):
                self.proc_qt.kill()
        self.proc_qt = None
        self._tail_timer.stop(); self.tail_view.setVisible(False)
        self.set_running(False)
        for i in range(self.queue_list.count()):
            roww = self._queue_item_row(i)
            if roww.status in {QStatus.RUNNING, QStatus.PAUSED}:
                roww.set_status(QStatus.PENDING)
        self._queue_urls = []; self._queue_index = 0
        self._pre_run_files = None; self._run_started_at = None; self._log_buffer = []
        self._paused = False; self.pause_btn.setText("Pause after current"); self.pause_btn.setEnabled(False)
        self.backoff_label.clear()
        self._clear_session()
    ...
    def _finish_queue(self):
        ...
        msg = (
            ...
            f"Suspects: {totals['suspect']}"
        )
        self._notify("Queue complete",
                     f"OK: {totals['ok']}  •  Fail: {totals['fail']}  •  Suspects: {totals['suspect']}",
                     7000)

        m = QMessageBox(self); m.setIcon(QMessageBox.Information)
        m.setWindowTitle("Queue complete"); m.setText(msg); m.exec()
    ...
    # ---------- Re-run helpers ----------
    def _rerun_from_history(self, kind="fail"):
        hist = self._load_history()
        urls = []
        for job in reversed(hist):
            if kind == "fail" and job.get("code", 0) != 0:
                u = job.get("input",""); 
                if u: urls.append(u)
                break
            if kind == "suspect" and job.get("suspect", 0) > 0:
                u = job.get("input",""); 
                if u: urls.append(u)
                break
        urls = [u for u in urls if u and SPOTIFY_URL_RE.match(u)]
        if not urls:
            QMessageBox.information(self, "Nothing to re-run", f"No {kind} jobs found in history.")
            return
        self._queue_clear()
        self._queue_add_urls(urls)
    ...
    def _session_file(self) -> Path | None:
        dest = self.dest_edit.text().strip()
        base = Path(dest) if dest else Path.cwd()
        try:
            p = base / "_logs" / "current_session.json"
    ...
    def _persist_session(self):
        ...
        try:
            data = {
                ...
                "extra": self.extra_args.text().strip(),
                "queue": self._queue_urls,
                "index": self._queue_index,
                "running": bool(self.proc_qt),
                "ts": time.time(),
            }
    ...
    def _maybe_offer_resume(self):
        ...
        try:
            ...
            return
        q = data.get("queue", []); idx = int(data.get("index", 0))
        if not q or idx >= len(q):
            return
        # Load fields
        self.dest_edit.setText(data.get("dest", self.dest_edit.text()))
        self.format_combo.setCurrentText(data.get("format", self.format_combo.currentText()))
        try: self.parallel_spin.setValue(int(data.get("parallel", self.parallel_spin.value())))
        except Exception: pass
        self.force_chk.setChecked(bool(data.get("force", self.force_chk.isChecked())))
        self.extra_args.setText(data.get("extra", self.extra_args.text()))

        # Offer resume
        resp = QMessageBox.question(self, "Resume previous session?",
                                    f"A previous session was found with {len(q)} URL(s). "
                                    f"Resume at item {idx+1}?",
                                    QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
        if resp == QMessageBox.Yes:
            self._queue_clear()
            self._queue_add_urls(q)
            self._queue_urls = q
    ...
    def on_error_qt(self, err):
        ...
        m.setWindowTitle("Failed to start"); m.setText(f"Error: {err}"); m.exec()

    def open_folder(self, folder: str):
        try:
            if sys.platform.startswith("win"): os.startfile(folder)  # type: ignore
    ...
    def closeEvent(self, event: QCloseEvent) -> None:
        if self.settings.value("minimize_to_tray", "true") == "true" and QSystemTrayIcon.isSystemTrayAvailable():
            event.ignore()
            self.hide()
            self._notify("Still running in tray", "Downloads continue in the background.", 4000)
        else:
            event.accept()
...
if __name__ == "__main__":
    # Windows: ensure toasts use our identity
    if platform.system() == "Windows":
        try:
