...
def get_app_icon() -> QIcon:
    base = Path(getattr(sys, "_MEIPASS", Path(__file__).parent))  # PyInstaller or source
    # Prefer .ico on Windows; fall back to .png anywhere
    for name in ( "spotify-dl-gui.ico", "spotify-dl-gui.png" ):
        p = base / name
        if p.exists():
            return QIcon(str(p))
    # last resort: a themed icon (Linux) or empty
...
class QueueRow(QWidget):
    def __init__(self, url: str, status: QStatus = QStatus.PENDING):
        ...
        self.lbl_url.setWordWrap(False); self.lbl_url.setMinimumWidth(280); self.lbl_url.setToolTip(url)
        ...
        row.addWidget(self.lbl_badge); row.addWidget(self.lbl_url, 1); row.addWidget(self.lbl_status)
...
class SpotifyDLGui(QWidget):
    def __init__(self):
        ...
        self.queue_list.setSpacing(6); self.queue_list.setMinimumWidth(380)
        ...
        # Run/Pause/Stop (now on the LEFT)
        ...
        if QSystemTrayIcon.isSystemTrayAvailable():
            ...
            self.tray.setIcon(get_app_icon())   # you can load a real icon here
            tray_menu = QMenu()
    ...
    def _poll_clipboard(self):
        ...
        if not self.auto_clip_chk.isChecked():
            return
        # Multiple URLs in clipboard? Try to split lines/spaces.
    ...
    def _start_next_job(self):
        ...
        if 0 <= self._queue_index < self.queue_list.count():
            ...
            roww.set_status(QStatus.RUNNING); self.queue_list.setCurrentRow(self._queue_index)
    ...
    def _tail_tick(self):
        ...
        try:
            ...
            if data:
                txt = data.decode("utf-8", "ignore").replace("\r", "\n")
                self.tail_view.moveCursor(self.tail_view.textCursor().End)
                self.tail_view.insertPlainText(txt)
        except Exception:
            pass
    ...
    def _rerun_from_history(self, kind="fail"):
        hist = self._load_history()
        urls = []
        # Collect last N relevant jobs (here: just the most recent one with the trait)
        for job in reversed(hist):
            if kind == "fail" and job.get("code", 0) != 0:
                urls.append(job.get("input",""))
                break
